...
这是一个面向 Agent、RAG 与 AI 平台场景的后端基础设施项目，基于 FastAPI，定位是可扩展的 Agent 后端骨架，而不是单一业务服务。当前能力包括 Web API、认证、异步数据库访问、Redis 缓存、Celery 任务、请求日志、第三方登录，以及可复用的向量检索抽象。
...
- `entrypoints/main.py`：FastAPI 推荐应用入口，导出 `app`。
