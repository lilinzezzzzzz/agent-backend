...
- 提供内部开放运行数据接口，只返回 RAG 运行数据，方便开放调用方自行分析或评分。
...
- 内部开放运行接口：返回稳定错误原因，并记录 `case_id`、`subject_user_id` 和请求范围。

内部开放运行接口也必须通过 `subject_user_id` 解析 allowed scope。若后续需要特权运行范围，必须显式增加内部专用字段并写审计记录，不能复用用户态请求字段绕过范围解析。
...
-> 内部开放运行 API，走 /v1/internal 签名认证

internal/schemas/rag.py
-> RAG answer、retrieve、external run data 的 request / response schema

internal/services/rag.py
-> RAG use case 编排：query prepare、filter、检索、rerank、context、生成、citation、trace
...
```text
request
-> prepare retrieval query: normalize + query rewrite / expansion
...
`prepare_retrieval_query()` 是查询前处理边界。首版至少做稳定 normalization，
并返回可观测的 `original_question`、`retrieval_query` 和 `expanded_queries`；
后续可以在不改主流程的前提下升级为 LLM query rewrite、multi-query、
query expansion 或 HyDE。该步骤只能优化检索表达，不能改变权限范围、
不能注入客户端未授权的 `domain/kb_id`。
...
不要混用这两个概念。API response 可以同时返回 `evidence_id` 和 `chunk_id`，但外部 gold evidence 应使用稳定 `chunk_id`，不要使用 run-scoped `evidence_id`。
...
- `text_hash` 由 Service 对 chunk text 或最终 excerpt 计算，供 trace、评分和回归核对。
...
该接口走内部签名认证，用于开放调用方同步获取单个 case 的 RAG 运行数据。服务端只执行 RAG 链路并返回数据，不计算评分指标、不判断 pass/fail、不实现评分算法。
...
请求不接收 `expected_answer`、`expected_chunk_ids`、评分 `mode` 或 `persist`。这些属于开放调用方的评分和分析逻辑，不进入本服务接口。
...
| `run_id` | RAG run ID |
| `trace_id` | 请求 trace |
| `user_id` | 用户态请求用户或开放运行 subject user |
| `original_question` | 用户原始问题，必要时脱敏 |
| `retrieval_query` | 经 `prepare_retrieval_query()` 处理后的检索 query |
| `expanded_queries` | query expansion / multi-query 产生的补充 query |
| `requested_domain` | 请求期望业务域 |
| `requested_kb_ids` | 请求期望知识库 ID |
| `allowed_domains` | 服务端解析出的可用业务域，建议只写入内部 trace |
| `allowed_kb_ids` | 服务端解析出的可用知识库 ID，建议只写入内部 trace |
| `effective_domains` | 最终检索业务域，建议只写入内部 trace |
| `effective_kb_ids` | 最终检索知识库 ID，建议只写入内部 trace |
...
- [ ] `prepare_retrieval_query()` 是否在检索前执行，并保留 `original_question/retrieval_query/expanded_queries` 以便 trace。
- [ ] query rewrite / expansion 是否只改变检索表达，不改变 requested scope、allowed scope 或最终权限 filter。
- [ ] 最终 vector filter 是否使用 `effective_domains/effective_kb_ids`，即 requested scope 和 allowed scope 的交集。
- [ ] 空交集是否按用户态拒答或内部开放运行稳定错误处理。
...
- [ ] Service 是否负责 query prepare、filter、检索、生成、citation、trace。
...
- [ ] trace 和内部开放运行数据响应是否包含 `evidence_id -> chunk_id/doc_id/kb_id/document_version/text_hash` 映射，且响应不暴露 `effective_domains/effective_kb_ids`。
...
- [ ] 内部开放运行数据接口是否只返回数据，不接收 gold answer/evidence，不计算 metrics，不判断 pass/fail。
...
2. 实现 `internal/services/rag.py` 的 `prepare_retrieval_query()` 和 `retrieve()`，替换订单/支付 mock 知识工具。
...
6. 增加 `POST /v1/internal/rag/external-runs`，只返回 RAG 运行数据。
7. 增加 RAG trace 持久化。
8. 配合外部质量回归流程用数据接口回归订单、支付、无证据、错误引用、prompt injection 负例。
