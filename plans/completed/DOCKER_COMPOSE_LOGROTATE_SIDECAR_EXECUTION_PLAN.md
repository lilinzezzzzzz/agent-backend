...
日志格式、应用责任边界和其他部署形态以
[日志输出与轮转部署方案](../../docs/logging/log_rotation.md)为准；本计划只负责 sidecar 的落地步骤和验收门槛。
...
plans/completed/DOCKER_COMPOSE_LOGROTATE_SIDECAR_EXECUTION_PLAN.md
```

`docs/README.md` 和 `plans/README.md` 已存在指向本计划的链接，经核对无需修改。
