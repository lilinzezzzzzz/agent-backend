...
TEST_TASK_ID = UUID("00000000-0000-7000-8000-000000001001")
TEST_SERVICE_ID = UUID("00000000-0000-7000-8000-000000001002")
mock_context.get_user_id = lambda: TEST_USER_ID

mock_logger = MagicMock()

# Mock UUIDv7 generator
_id_counter = 0
...
mock_ids_module.uuid7_unique_id = mock_gen_id
...
async def test_create_and_insert_strictness(user_dao, db_session):
    ...
    assert user.creator_id == TEST_USER_ID
...
async def test_update_strictness(user_dao, db_session):
    ...
    assert reloaded.updater_id == TEST_USER_ID
...
async def test_explicit_audit_actor_works_without_request_context(
    ...
    user = user_dao.create(
        audit_actor=AuditActor.task(TEST_TASK_ID),
        username="worker_created",
    )
    assert user.creator_id == TEST_TASK_ID
    ...
    await user_dao.update(
        ...
        audit_actor=AuditActor.service(TEST_SERVICE_ID),
    )

    reloaded = await user_dao.query_by_primary_id(user.id)
    assert reloaded is not None
    assert reloaded.updater_id == TEST_SERVICE_ID
